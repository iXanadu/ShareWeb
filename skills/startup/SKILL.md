---
name: startup
description: When a new session begins in this FastAPI project, run this to orient on project state, memory, service status, and recent work before doing anything else.
---
...
still contains the `[Project Name]` placeholder, or `pyproject.toml` still contains the `projectname` placeholder.
...
3. `memory_search` query="migration schema endpoint deployment" scope=project limit=5 — schema, API surface and deploy state
4. `memory_search` query="wip current working" scope=project limit=5 — active work

### Shared + machine
5. `memory_search` scope=shared limit=5 — cross-project lessons (cross-project FastAPI/asyncpg/Python lessons)
...
git status
git log --oneline -10
python -c 'from server.main import app' && echo 'app imports OK'
```
...
- Summarize current status (code, endpoints, deployment)

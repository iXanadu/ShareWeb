...
Resolved tokens cache at `sh:atok:{sha256hex}` for 30 seconds. **Revocation bypasses the cache**
with an immediate `DEL`. Share-link tokens use a separate `sh:ltok:` namespace (§3.10) — two
credential classes never share a cache namespace, whatever their hashes look like.
...
The three calls the approval screen (§11.26) makes, all session-authenticated:
...
POST /api/v1/auth/device/approve  { "userCode": "QRTZ-8H4M" }
 ← 200 { tokenId: "shr_01J…", name: "…", scopes: [...] }      ← never the token itself
POST /api/v1/auth/device/deny     { "userCode": "QRTZ-8H4M" }
 ← 200 { }
```

**Approve does not return the token.** It is issued server-side and delivered only to the polling
agent, so the secret never touches the browser that authorised it. Approval audits
...
- Cookie `share_r_{first 8 chars of the token}`, `Path=/s/{token}`, `HttpOnly`, `Secure`,
  ...
  cookies and none is ever sent to another link. Link creation **retries token generation while
  that 8-character prefix already exists among the instance's live links**, so two live links can
  never produce two same-named cookies at overlapping paths — collision is impossible rather than
  merely unlikely (58⁸ ≈ 1.3 × 10¹⁴).
- Value is `{recipientSessionId}.{HMAC-SHA256(secret_key, id)}`. The MAC is checked first
  (cheap, constant-time), then the row, cached at `sh:rcp:{id}` for 300 s.

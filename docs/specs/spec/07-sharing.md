...
Generation retries while the token's first 8 characters already appear among the instance's live
links, because those 8 characters are both the stored `display_prefix` and the recipient cookie's
name (§4.7); two live links sharing them would mean two same-named cookies at overlapping paths.
...
`password` on `PATCH` takes three values: a **string** sets your own, **`true`** has the server
generate one and return it in that response only, and **`null`** removes the password entirely.
All three revoke every recipient session on the link immediately (P9) and audit as
`link.password_change`. This is what makes a lost generated password recoverable without changing
the URL a client already holds.
